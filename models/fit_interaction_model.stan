// =============================================================================
// fit_interaction_model.stan
//
// Model for Section 5.2: Mother's IQ vs. Child Test Scores with group-specific
// nonlinear interaction. Single predictor x (mother's IQ), binary group z in
...
parameters {
  ...
  real<lower=0> lambda_main;
  real<lower=1e-12> delta_main;
...
transformed parameters {
...

  {
    ...
    lprior += student_t_lpdf(delta_main | df_delta, 0, s_eff)
              - student_t_lccdf(0 | df_delta, 0, s_eff);
    if (d_order > 0)
      lprior += d_order * log(delta_main);
  ...
  lprior += inv_gamma_lpdf(lambda_main | 3, 0.8);
