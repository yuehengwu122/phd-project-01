...
<div class="finding">
Curves stabilize around eps = 0.2. At smaller eps, the raw method shows Inf (no posterior draws below eps). The logspline method is smoother but underestimates BF at large delta due to boundary extrapolation.
</div>
...
<div class="finding">
Convergence eps increases with delta (posterior moves away from zero) and with prior order (non-local priors push mass away from zero). At delta = 0, convergence eps is near 0.01 for all priors and N.
</div>
...
<div class="finding">
Logspline gives graded, finite evidence across all settings. Prior order effect is visible: d=0 (local) gives strongest BF10, d=3 weakest. Raw method saturates at Inf for delta >= 1 at small N, but gives finite values at N = 200 for delta = 1.
...
<div class="finding">
Where both methods are finite, they agree well (points near the diagonal). The raw method is unbiased but has lower resolution; logspline provides smoother estimates but can be biased near the boundary.
</div>
...
<div class="finding">
CV-optimal eps ranges from ~0.05 (delta=0, strong posterior mass near zero) to ~0.4 (delta=3, posterior far from zero). The interval BF at CV-optimal eps tracks bridge sampling well in moderate regimes but remains conservative for large effects.
</div>
