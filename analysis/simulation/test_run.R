#' Smoke test for Experiment 3 on Saturn Cloud (minimal run)
#'
#' Purpose:
#' - Verify environment/package/model compilation works
#' - Verify one end-to-end fit can run and write outputs
#'
#' Minimal settings:
#' sample_sizes <- c(20)
#' delta_values <- c(0)
...
chains <- min(2L, max(1L, cpu_available))
cores_use <- chains

rstan_options(auto_write = TRUE)
options(mc.cores = cores_use)

message(sprintf("Smoke test CPUs: %d | chains=%d | cores=%d", cpu_available, chains, cores_use))
...
eps_grid <- seq(0.05, 0.5, length.out = 10)
...
# ============================================
# PATHS (smoke-test specific)
# ============================================

dir.create("data/simulated", recursive = TRUE, showWarnings = FALSE)

bf_interval_path <- "data/simulated/smoke_test_bf_interval.csv"
bf_bridge_path <- "data/simulated/smoke_test_bf_bridge.csv"
posterior_path <- "data/simulated/smoke_test_posterior_draws.csv"
summary_path <- "data/simulated/smoke_test_bf_summary.csv"
completed_path <- "data/simulated/smoke_test_completed_configs.csv"
...
# reset smoke-test outputs each run
for (p in c(bf_interval_path, bf_bridge_path, posterior_path, summary_path, completed_path)) {
  if (file.exists(p)) {
    file.remove(p)
  }
}
  ...
  for (N in sample_sizes) {
    ...
    for (d_order in nlp_orders) {
      for (rep_id in seq_len(n_replicates)) {
        ...
        if (length(delta_draws) == 0) {
          stop("No posterior draws found for parameter 'delta' in fit_alt.")
        }
        ...
        tryCatch(
          {
            bridge_alt <- bridgesampling::bridge_sampler(fit_alt, method = "normal", verbose = FALSE)
            bridge_null <- bridgesampling::bridge_sampler(fit_null, method = "normal", verbose = FALSE)

            logml_alt <- as.numeric(bridge_alt$logml)[1]
            logml_null <- as.numeric(bridge_null$logml)[1]

            if (!is.finite(logml_alt) || !is.finite(logml_null)) {
              stop("Bridge sampling returned non-finite or empty logml.")
            }
            ...
            bf_bridge_df <- data.frame(
              ...
              order = paste0("d", d_order),
              replicate = rep_id,
              log_bf10_bridge = logml_alt - logml_null,
              bf10_bridge = exp(logml_alt - logml_null),
              bridge_alt_logml = logml_alt,
              bridge_null_logml = logml_null,
              stringsAsFactors = FALSE
            )
          ...
          error = function(e) {
            bridge_ok <- FALSE
            message(sprintf("Smoke test bridge sampling failed: %s", conditionMessage(e)))
          }
        )
