#' Simulation Experiment 3: local-prior-only run with bridge sampling
#'
#' Current scope:
#' - Fits only the local prior model (d_order = 0)
#' - Restricts sample sizes to N = 20, 50, 100, 200
#' - Saves all posterior delta draws for later reweighting
#' - Computes bridge-sampling BF against the linear null model
#' - Writes to local-only output files so results do not mix with older runs
...
message(sprintf(
  "Detected CPUs: %d | chains=%d | using cores=%d",
...
bf_bridge_path   <- "data/simulated/results_sim3_rstan_bf_bridge_local_small.csv"
posterior_path   <- "data/simulated/results_sim3_rstan_posterior_draws_local_small.csv"
completed_path   <- "data/simulated/results_sim3_rstan_completed_local_small.csv"
...
total_configs <- length(delta_values) * length(sample_sizes) * n_replicates
counter <- 0L
...
for (delta in delta_values) {
  ...
  for (rep_id in seq_len(n_replicates)) {
    ...
    for (N in sample_sizes) {
      ...
      message(sprintf(
        "[%d/%d] delta=%s N=%d d=%d rep=%d",
        counter, total_configs, delta, N, target_d_order, rep_id
      ))
      ...
      tryCatch({
        ...
        message(sprintf(
          ...
          delta, N, target_d_order, rep_id, conditionMessage(e)
        ))
      })
