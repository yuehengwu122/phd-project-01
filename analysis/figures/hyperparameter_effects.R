...
plot_data$delta <- factor(
  plot_data$delta,
  levels = paste0("δ = ", delta_values)
)
plot_data$lambda <- factor(
  plot_data$lambda,
...
ggsave(
  "analysis/figures/output/gp_hyperparameter_grid.png",
  gp_effects_plot,
  width = 12,
  height = 10,
  dpi = 300
...
create_interpretation_plots <- function() {
  ...
  p2 <- ggplot(lambda_demo_data, aes(x = x, y = y, color = lambda)) +
    ...
    labs(
      title = "Effect of Length Scale (λ)",
