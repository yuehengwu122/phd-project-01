...
plot_prior_comparison_ggplot <- function(output_path = NULL) {
  ...
  prior1_var <- (2 * dt(sqrt(x_vals) / 2.7, df = 4) / 2.7) /
    (2 * pmax(sqrt(x_vals), 1e-12))
  ...
  p1 <- ggplot(df_orig, aes(x = x, y = density, color = prior)) +
    ...
    scale_color_manual(
      values = c("Prior 1" = "green4", "Prior 2" = "red3"),
      labels = c(
        expression("Prior 1: " * delta ~ " ~ t"[4]^"+" * "(s=2.7)"),
...
plot_gg <- plot_prior_comparison_ggplot(
  "analysis/figures/output/plot-prior-delta.png"
)
print(plot_gg)
