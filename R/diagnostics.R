...
print_quick_summary <- function(
  ...
  cat(
    "Fitting time:",
    round((elapsed[1, 1] + elapsed[1, 2]) / 3600, 2),
    "hours\n"
  )
  cat("\n")
  ...
  diag_table <- data.frame(
    ...
    "Min n_eff/N" = sprintf(
      "%.4f",
      min(summary_df[, "n_eff"] / nrow(rstan::extract(fit)$lp__), na.rm = TRUE)
    ),
    "Divergences" = sprintf("%d", sum(rstan::get_num_divergent(fit))),
    "Max treedepth" = sprintf("%d", sum(rstan::get_num_max_treedepth(fit))),
    "Mean accept_stat" = sprintf(
      "%.4f",
      mean(accept_stat_values, na.rm = TRUE)
    ),
    row.names = "Value",
    check.names = FALSE
  )
  ...
  if (unstandardize) {
    cat("\n")
    cat(
      "Unstandardized linear coefficients (beta1_unstd = theta * sigma / x_range):\n"
    )
    ...
    if (is.null(post$theta) || is.null(post$sigma)) {
      ...
      if (is.null(X)) {
        ...
        if (is.matrix(theta_draws)) {
          ...
          beta1_unstd <- matrix(
            theta_draws * sigma_draws / x_ranges[1],
            ncol = 1
          )
        }
...
summary_gp_fit <- function(
  ...
  if (max_rhat > 1.01 || n_div > 0) {
    ...
    if (max_rhat > 1.01) {
      cat("  Warning: Some Rhat > 1.01\n")
    }
    if (n_div > 0) {
      cat("  Warning:", n_div, "divergent transitions\n")
    }
    cat("\n")
  }
  ...
  if (is.vector(X_scaled)) {
    X_scaled <- matrix(X_scaled, ncol = 1)
  }
  P <- ncol(X_scaled)
  ...
  if (is.null(X_orig)) {
    X_orig <- X_scaled
    warning(
      "X_orig not found in result; using scaled X. Unstandardized coefficients may be incorrect."
    )
  }
  if (is.vector(X_orig)) {
    X_orig <- matrix(X_orig, ncol = 1)
  }

  # Get delta_prior from result (stored by fit_model)
  delta_prior <- result$delta_prior
  if (is.null(delta_prior)) {
    delta_prior <- "p1"
  }
  ...
  log_sdr_linear <- ifelse(
    is.na(sdr_linear) | sdr_linear <= 0,
    NA,
    log10(sdr_linear)
  )
  ...
  predictor_rows <- data.frame(
    ...
    logSDR10 = ifelse(
      is.na(log_sdr_linear),
      "—",
      sprintf("%.2f", log_sdr_linear)
    ),
    check.names = FALSE
  )
  ...
  log_sdr_nonlinear <- ifelse(
    is.na(sdr_nonlinear) | sdr_nonlinear <= 0,
    NA,
    log10(sdr_nonlinear)
  )
  ...
  nonlinear_table <- data.frame(
    ...
    logSDR10 = ifelse(
      is.na(log_sdr_nonlinear),
      "—",
      sprintf("%.2f", log_sdr_nonlinear)
    ),
    check.names = FALSE
  )
  ...
  if (compute_sdr) {
    cat(
      "logSDR10: log10 of Savage-Dickey Ratio (evidence for effect vs. no effect)\n"
    )
  }
  cat(paste0(rep("=", 50), collapse = ""), "\n")
...
plot_posterior_delta <- function(
  ...
  if (is.matrix(draws)) {
    ...
    if (plot_prior) {
      prior_df <- do.call(
        rbind,
        lapply(predictor_names, function(pn) {
          x_range <- range(
            draws_df$value[draws_df$predictor == pn],
            na.rm = TRUE
          )
          x_seq <- seq(0, x_range[2], length.out = 200)
          data.frame(
            x = x_seq,
            y = half_t_density(x_seq),
...
plot_posterior_theta <- function(
  ...
  if (trim_quantile < 1) {
    ...
    if (is.matrix(draws)) {
      ...
      draws <- draws[
        draws >= quantile(draws, lower_q) & draws <= quantile(draws, upper_q)
      ]
    }
  }
  ...
  if (is.matrix(draws)) {
    ...
    if (plot_prior) {
      ...
      prior_df <- do.call(
        rbind,
        lapply(seq_along(predictor_names), function(j) {
    ...
    if (plot_prior) {
      ...
      prior_df <- data.frame(
        x = x_seq,
        y = dcauchy(x_seq, location = 0, scale = scale_theta)
      )
      p <- p +
        geom_line(
          data = prior_df,
...
plot_gp_trends <- function(
  ...
  # Keep dimnames (avoid as.matrix() dropping/altering colnames in edge cases)
  X_s <- result$data$X
  ...
  # Force predictor names from the object (ignore external / stale names)
  predictor_names <- colnames(X_orig)
  if (is.null(predictor_names)) {
  ...
  # original-scale slopes if available (for linear trend)
  beta1_orig_draw <- NULL
  if (!is.null(result$coef_orig$beta1_draws)) {
    beta1_orig_draw <- result$coef_orig$beta1_draws[idx, , drop = FALSE]
  }

  x_bar_orig <- colMeans(X_orig)

  # scaling map: x_s = (x - min)/range - 0.5  (your convention B)
  x_min <- apply(X_orig, 2, min)
  ...
  for (kk in seq_along(p_idx)) {
    ...
    for (s in 1:nd) {
      sigma <- sigma_draw[s]
      beta1 <- sigma * theta_draw[s, ] # centered-rescaled slope (for residuals)
      ...
      if (!is.null(beta1_orig_draw)) {
        lin <- y_bar + beta1_orig_draw[s, i] * (xg_orig - x_bar_orig[i])
      } else {
        # fallback: linear part on centered-rescaled scale
    ...
    out_list[[kk]] <- data.frame(
      predictor = x_name, # keep as character; factor later
      x = xg_orig,
  ...
  # now apply factor with correct level order
  df$predictor <- factor(df$predictor, levels = predictor_names[p_idx])
  raw$predictor <- factor(raw$predictor, levels = predictor_names[p_idx])
...
plot_hsgp_trends <- function(
  ...
  post <- rstan::extract(
    fit,
    pars = c("beta0", "sigma", "theta", "delta", "lambda", "z_basis")
  )
  S <- length(post$beta0)
  if (S > ndraws) {
    set.seed(seed)
  ...
  p <- ggplot() +
    ...
    geom_ribbon(
      data = df,
      aes(x = x, ymin = full_lo, ymax = full_hi),
      fill = "lightskyblue",
      alpha = 0.30
    ) +
    geom_line(
