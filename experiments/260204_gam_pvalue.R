...
hist(pvals_penalty, freq = FALSE, main = "Histogram of p-values with penalty", xlab = "p-value")
...
hist(pvals_gam, freq = FALSE, main = "Histogram of p-values (gam package)", xlab = "p-value")
mean(pvals_gam < 0.05)
...
pvals_linear_mgcv <- pvals_linear_mgcv[!is.na(pvals_linear_mgcv) & pvals_linear_mgcv > 0]

pvals_nonlinear_mgcv <- unlist(lapply(pvals_mgcv, `[[`, "nonlinear_p"))
pvals_nonlinear_mgcv <- pvals_nonlinear_mgcv[!is.na(pvals_nonlinear_mgcv) & pvals_nonlinear_mgcv > 0]
...
pvals_linear_gam <- pvals_linear_gam[!is.na(pvals_linear_gam) & pvals_linear_gam > 0]

pvals_nonlinear_gam <- unlist(lapply(pvals_gam, `[[`, "nonlinear_p"))
pvals_nonlinear_gam <- pvals_nonlinear_gam[!is.na(pvals_nonlinear_gam) & pvals_nonlinear_gam > 0]

# Compare histograms of p-values
par(mfrow = c(1, 2))
hist(pvals_linear_mgcv, freq = FALSE, main = "mgcv: Linear Effect (Null)", xlab = "p-value")
hist(pvals_linear_gam, freq = FALSE, main = "gam: Linear Effect (Null)", xlab = "p-value")
...
cat("Power to detect nonlinear effect (mgcv):", mean(pvals_nonlinear_mgcv < 0.05), "\n")
cat("Power to detect nonlinear effect (gam):", mean(pvals_nonlinear_gam < 0.05), "\n")
...
bootstrap_gam_analysis <- function(gam_data, dataset_name, n_bootstrap = 100) {
  ...
  cat("Starting bootstrap analysis for", dataset_name, "with", n_bootstrap, "replicates...\n")
  ...
  for (b in 1:n_bootstrap) {
    ...
    tryCatch({
      ...
      for (pred in predictors) {
        ...
        if (!is.na(linear_p) && linear_p > 0) {
          results_mgcv[pred, "linear_sig_rate"] <- results_mgcv[pred, "linear_sig_rate"] + (linear_p < 0.05)
        }
        if (!is.na(nonlinear_p) && nonlinear_p > 0) {
          results_mgcv[pred, "nonlinear_sig_rate"] <- results_mgcv[pred, "nonlinear_sig_rate"] + (nonlinear_p < 0.05)
        }
      }
    }, error = function(e) {
    ...
    tryCatch({
      ...
      for (i in seq_along(predictors)) {
        ...
        if (!is.na(linear_p) && linear_p > 0) {
          results_gam[pred, "linear_sig_rate"] <- results_gam[pred, "linear_sig_rate"] + (linear_p < 0.05)
        }
        if (!is.na(nonlinear_p) && nonlinear_p > 0) {
          results_gam[pred, "nonlinear_sig_rate"] <- results_gam[pred, "nonlinear_sig_rate"] + (nonlinear_p < 0.05)
        }
      }
    }, error = function(e) {
