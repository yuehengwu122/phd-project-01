...
for (pred in predictor_names) {
  ...
  newdata <- data.frame(matrix(
    rep(colMeans(gam_data[, -1]), each = 100),
    nrow = 100,
    byrow = FALSE
  ))
...
for (i in 1:length(predictor_names)) {
  ...
  x_seq <- seq(
    min(gam_data[[pred_name]]),
    max(gam_data[[pred_name]]),
    length.out = 100
  )
...
gam_fit_time <- system.time({
  ...
  for (col in predictor_cols) {
    gam_data_centered[[paste0(col, "_c")]] <- gam_data[[col]] -
      mean(gam_data[[col]])
  }
