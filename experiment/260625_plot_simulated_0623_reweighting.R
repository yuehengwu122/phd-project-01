...
local_data_dir <- "data/simulated_0623"
legacy_data_dir <- "data/simulated"
output_dir <- "results/simulated_0623"
...
message("Loading local-only bridge and posterior draws from simulated_0623...")
...
p_bridge_legacy <- make_bf_row_plot(
  ...
  title_text = "Legacy direct bridge sampling (older model; only partial runs available)",
...
p_logspline_reweighted <- make_bf_row_plot(
  ...
  title_text = sprintf(
    "Reweighted interval-null BF from d=0 posterior [logspline, eps = %.1f]",
    eps_fixed
  ),
  y_label = "Median log BF10"
)
...
pdf_path <- file.path(output_dir, "sim3_reweighting_0623_overview.pdf")
png_path <- file.path(output_dir, "sim3_reweighting_0623_overview.png")
main_pdf_path <- file.path(output_dir, "sim3_reweighting_0623_logspline_only.pdf")
main_png_path <- file.path(output_dir, "sim3_reweighting_0623_logspline_only.png")

ggsave(pdf_path, combined_plot, width = 12, height = 7.5)
ggsave(png_path, combined_plot, width = 12, height = 7.5, dpi = 300)
ggsave(main_pdf_path, p_logspline_reweighted, width = 12, height = 3.8)
ggsave(main_png_path, p_logspline_reweighted, width = 12, height = 3.8, dpi = 300)

message("Saved plot to:")
message("  ", pdf_path)
message("  ", png_path)
message("  ", main_pdf_path)
message("  ", main_png_path)
