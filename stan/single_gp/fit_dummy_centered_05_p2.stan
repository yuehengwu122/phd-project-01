...
parameters {
  ...
  real<lower=0> delta2_g;       // standardized GP amplitude for g

  // interaction GP h(x)
  real<lower=0> lambda_h;      // length-scale for h
  real<lower=0> delta2_h;       // standardized GP amplitude for h
...
transformed parameters {
  ...
  real<lower=0> alpha_g = sqrt(delta2_g) * sigma;
  real<lower=0> alpha_h = sqrt(delta2_h) * sigma;
  
  // Prior buffer (for bridge sampling)
  real lprior = 0;

  // half-t priors via truncated Student-t
  lprior += student_t_lpdf(delta2_g | 1.15, 0, 7) - student_t_lccdf(0 | 1.15, 0, 7);
  lprior += inv_gamma_lpdf(lambda_g | 3, 0.8);

  lprior += student_t_lpdf(delta2_h | 1.15, 0, 7) - student_t_lccdf(0 | 1.15, 0, 7);
...
generated quantities {
...

  {
    ...
    matrix[50,N] K_no_h;
    {
      real a2_g = square(alpha_g);
      real a2_h = square(alpha_h);
      for (g in 1:50)
        for (n in 1:N) {
          real xc_g = X_new[g,2];
          real xc_n = X[n,2];
