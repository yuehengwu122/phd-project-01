...
data {
  ...
  int<lower=0> d_order;         // moment order d = 0,1,2,3,...
  real<lower=0> kappa_delta;     // median-matching scale for this d
  int<lower=1> df_delta;         // df of half-t base
  real<lower=0> s_delta;         // scale of half-t base
  real logC_base;                // log E_g[delta^d_order] under base half-t+(df_delta, s_delta), kappa=1
}
...
parameters {
  ...
  // ---- sample delta directly (avoid exactly 0 for d>0) ----
...
transformed parameters {
  ...
  // ---- moment prior on delta (no Jacobian needed) ----
  {
